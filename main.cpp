...
void printHelp(int argc, ...) {
    if (argc > 0) {
        ...
        for (int i = 0; i < argc; i++) {
            const char* arg = va_arg(args, const char*);  // Get each argument as a const char*
            // Print the specific help for this argument
            if (helpArgs.find(arg) != helpArgs.end()) {
                cout << "USAGE: " << prog_name << " " << arg << " " << helpArgs[arg] << "\n";
            }
            else {
...
{
	if (argv[0][0] != NULL) // from standard ISO C11: argv[0][0] shall be the null character if the program name is not available from the host environment.
	{
		char *last_slash;
	...
	if(result == ERR_IMPROPER_ARGS)
		cerr << "The routine failed with error: improper arguments.\n"
	         << "RUN " << prog_name << " help " << request << "to check whether all arguments were passed.";
